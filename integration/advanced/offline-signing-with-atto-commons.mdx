---
title: Advanced Atto Commons Integration — Keys and Transactions
sidebar_label: Advanced Commons integration
description: Build an Atto integration with Commons examples for key generation, addresses, signed transactions, sending, receiving and network monitoring.
---
...
# Advanced Atto Commons integration {#offline-signing-with-atto-commons}
...
construct and sign transactions, and interact directly with an Atto node – all without relying on the Wallet Server
component. This is ideal for advanced integrators (such as exchanges or custodians) who require full control over key
management and transaction authorization.

Key generation and signing happen locally, while the examples use online clients for account data, work and publication. If you need air-gapped signing, keep the signing component separate from those clients. The [protocol serialization and signing reference](/docs/integration/advanced/protocol-offline-signing-reference) explains the byte-level formats.

**Note:** Atto uses Ed25519 elliptic curve cryptography for its account keys and signatures. If you use a third-party signer or HSM, check both its Ed25519 support and how you will handle Atto key derivation and transactions. In this guide, we use Atto Commons for all key and transaction operations for consistency and simplicity.
...
The JavaScript examples use Commons 7.0.2. Keep their packages on matching versions, and check the JVM examples against the release you install. For a shorter introduction, start with the [Commons getting-started guide](/docs/integration/commons).

<Tabs groupId="library">
  <TabItem value="js" label="JavaScript">

    ```bash
      npm i @attocash/commons-core@7.0.2 \
        @attocash/commons-node@7.0.2 \
        @attocash/commons-node-remote@7.0.2 \
        @attocash/commons-wallet@7.0.2 \
        @attocash/commons-worker-remote@7.0.2
    ```
...
Use a disposable wallet while following these examples. Never log recovery phrases or private keys, and do not fund a wallet whose secrets have appeared in logs, screenshots or shared output. The examples below print only public values.

The first step is to generate a private key for your Atto account. The Atto Commons library provides utilities for
secure key generation. In JavaScript, `await AttoMnemonic.generate()` creates a fresh mnemonic that can derive indexed
private keys. Kotlin also exposes secure key generation through Atto Commons. You can import an existing private key
from a hex string if you have one (for example, from a mnemonic or another source).
...
<Tabs groupId="library">
  <TabItem value="js" label="JavaScript">
    ...
    (async () => {
      ...
      const privateKey0 = await seed.toPrivateKey(index0);

      // Derive the public key if needed
      const publicKey0 = await privateKey0.toPublicKey();
  ...
  <TabItem value="java" label="Java">
    ...
    System.out.println("Public Key: " + publicKey0);
    ```
...
mnemonic seed, ensure you use a secure, Atto-compatible method, as illustrated above. Do not expose the mnemonic, seed or private key in application output.
...
<Tabs groupId="library">
  <TabItem value="js" label="JavaScript">
    ...
    ```javascript
    import { AttoAddress, AttoAlgorithm } from '@attocash/commons-core';

    // Assume we have a private key from the previous section (e.g., privateKey0)
    const publicKey = await privateKey0.toPublicKey(); // derive Ed25519 public key
    const address = publicKey.toAddress(AttoAlgorithm.V1); // build the Atto address
    console.log('Address:', address.toString());
    console.log('Parsed address:', AttoAddress.parse(address.toString()).toString());
    ```
...
<Tabs groupId="library">
  ...
  <TabItem value="kotlin" label="Kotlin">
    ...
    // Remote clients; verify these APIs against your installed JVM release
...
`client.accountByAddresses([...addresses])`. The result will include each account’s current balance, frontier (last block hash),
...
<Tabs groupId="library">
  <TabItem value="js" label="JavaScript">
    ...
    const accountsInfo = await nodeClient.accountByAddresses(addressesToMonitor);
  ...
  <TabItem value="java" label="Java">
    ...
    Collection<AttoAccount> accountsInfo = nodeClient.account(addressesToMonitor).get();
...
<Tabs groupId="library">
  ...
  <TabItem value="java" label="Java">
    ...
    kotlin.jvm.functions.Function0<AttoAddress> defaultRepresentativeProvider = () -> {
...
<Tabs groupId="library">
  <TabItem value="js" label="JavaScript">
    ...
    ```javascript
    import { AttoAmount, AttoUnit, toAttoIndex } from '@attocash/commons-core';
    ...
    const recipientAddress = await wallet.getAddress(recipientIndex);
    const tx = await wallet.sendByIndex(index0, recipientAddress, amount, null);
...
<Tabs groupId="library">
  <TabItem value="js" label="JavaScript">
    ...
    // Call this during shutdown and await both cancellations.
    async function stopMonitoring() {
      await txJob.cancelAndJoin();
      await entryJob.cancelAndJoin();
    }
    ```
...
With Atto Commons for JavaScript/TypeScript, Kotlin and Java, you can manage the transaction lifecycle in your own code: generate keys, construct and sign transactions, publish them, and react to incoming funds. You control key storage and payment authorization, with more flexibility and more implementation work than a high-level Wallet Server integration.

Keep `accountMap` up to date, store private keys securely, and check the original transaction hash after a lost response before creating another payment. Running your own node and work server gives you control over those dependencies. If your signing keys must stay offline, separate signing from the online clients used in this tutorial.
